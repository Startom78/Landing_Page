...
function isBirthdateNotFuture(element) {
    // Ici on vérifie si l'utilisateur n'a pas rentré une date dans le futur
...
function isDateMajor(element) {
    // Ici cette fonction me permet de vérifier si l'utilisateur est majeur ou non
    ...
    if (
        month < 0 ||
        (month === 0 && date.getDate() < userBirthdate.getDate())
    ) {
        age--;
    }
    if (age < 18) {
...
function validateIntegerOnly(element) {
    // Cette fonction permet de vérifier que le nombre de tournoi auquel l'utilisateur a participé est un entier
...
export const initFormValidation = () => {
    ...
    form.addEventListener("submit", (event) => {
        ...
        if (
            validators.filter((validator) => validateField(validator))
                .length === 0
        ) {
            const thanks = document.querySelector(".thanks");
            const closeRedButton = thanks.querySelector("button");
            form.style.display = "none";
            thanks.style.display = "flex";
            form.reset();

            closeRedButton.addEventListener("click", () => {
